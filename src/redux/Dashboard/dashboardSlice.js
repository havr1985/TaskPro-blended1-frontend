  ...
  extraReducers: (builder) =>
    builder
      ...
      .addCase(updateCardStatus.fulfilled, (state, action) => {
        ...
        // 	);
        // console.log(state.currentDashboard.column[currentColumnIdx].card);
        ...
        const { result, newCardIdx, currentColumnId } = action.payload;
        const newColumnId = result.owner;
        const currentColumnIdx = state.currentDashboard.column.findIndex(
          (item) => item._id === currentColumnId
        );
        const newColumnIdx = state.currentDashboard.column.findIndex(
        ...
        const cardStateIdx = state.currentDashboard.column[
          newColumnIdx
        ].card.findIndex((item) => item._id === result._id);
        console.log(cardStateIdx);

        if (isCardInNewColumn && cardStateIdx === newCardIdx) {
          return;
